      ...
      <div className="p-3 max-h-[calc(100vh-200px)] overflow-y-auto custom-scrollbar">
        <div className="flex flex-col gap-1">
          <Link href={`/books`} className="w-full">
            <button className="w-full text-left px-4 py-3 rounded-xl transition-all duration-200 hover:bg-primary/10 hover:pl-5 hover:text-primary font-medium text-base-content/80 group flex items-center justify-between">
              <span>All Books</span>
          ...
          {Categories.map((category, index) => (
            <Link key={`${category}-${index}`} href={`/books?category=${category}`} className="w-full">
              <button className="w-full text-left px-4 py-3 rounded-xl transition-all duration-200 hover:bg-primary/10 hover:pl-5 hover:text-primary font-medium text-base-content/80 group flex items-center justify-between">
                <span>{category}</span>
                <span className="opacity-0 group-hover:opacity-100 transition-opacity text-primary">
                  →
